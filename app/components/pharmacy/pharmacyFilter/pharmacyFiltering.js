...
import FilteringStore from '../../../containers/pharmacyStore/pharmacyFiltering/pharmacyFilterlingStore';

const CheckboxGroup = Checkbox.Group;
const plainOptions = ['English', 'Chinese', 'Japanese'];
export default function pharmacyFiltering() {
  const [checkedList, setCheckedList] = FilteringStore((state) => [state.checkedList, state.setCheckedList]);
  const onChange = (list) => {
    setCheckedList(list);
  };
  return (
    ...
    >
      ...
      >
        <div className="flex-col justify-start gap-[19px] inline-flex">
          <div
            className="pt-5 pl-[5px] justify-start items-start text-lime-500 text-2xl font-bold font-['Pretendard']"
          >
            Language
          </div>
